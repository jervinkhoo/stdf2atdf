# src/core/atdf/utils.py

def process_pass_fail_flag(stdf_values):
...
def process_alarm_flags(stdf_values):
    test_flg, parm_flg = int(stdf_values[0], 2), int(stdf_values[1], 2)
    flags = {
        'A': (test_flg >> 0) & 1,
        'D': (parm_flg >> 1) & 1,
...
def process_state_field(stdf_values):
    """
    Process state fields from STDF to ATDF format.
    Combines characters by pairs or formats single characters into comma-separated strings.
...
def process_data_file_type(_):
    return 'A'

def process_pass_fail_code(stdf_value):
...
def process_retest_code(stdf_value):
    binary_value = int(stdf_value, 2)
...
def process_ftr_alarm_flags(stdf_value):
    binary_value = int(stdf_value, 2)
    flags = {
        'A': (binary_value >> 0) & 1,
        'N': (binary_value >> 4) & 1,
...
def process_ftr_relative_address(stdf_value):
    return hex(stdf_value)[2:] if isinstance(stdf_value, int) else None

def process_generic_data(stdf_value):
    return '|'.join(stdf_value)

def process_mode_array(stdf_value):
    return ','.join(hex(num)[2:] for num in stdf_value)

def process_radix_array(stdf_value):
